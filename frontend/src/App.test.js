...
beforeEach(() => {
  ...
  global.fetch = jest.fn((address, options = {}) => {
    ...
    if (url.pathname.endsWith('/auth/login')) return json({ access_token: `header.${btoa(JSON.stringify({ exp: Math.floor(Date.now() / 1000) + 3600 }))}.signature`, token_type: 'bearer' });
...
test('denied staff actions show the permission error and expired sessions disable writes', async () => {
  ...
  let status = 403;
  global.fetch = jest.fn((address, options) => address.endsWith('/vehicles')

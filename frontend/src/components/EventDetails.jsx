...
import { canReviewEvents, canViewEvidence } from '../lib/permissions';
...
export default function EventDetails({ event, demo, session, onClose, onSignIn, onAuthError, onChange, revision }) {
  ...
  const [detailError, setDetailError] = useState(''), [department, setDepartment] = useState(event.assigned_department || ''), [departmentDirty, setDepartmentDirty] = useState(false);
  useEffect(() => { if (!departmentDirty) setDepartment(item.assigned_department || ''); }, [item.assigned_department, departmentDirty]);
  useEffect(() => {
    if (demo) return undefined;
    const controller = new AbortController();
    setLoading(true);
    api(`/events/${event.id}`, { signal: controller.signal }).then(data => { if (!controller.signal.aborted) { setItem(data); setDetailError(''); } })
  ...
  const assignDepartment = async e => {
    e.preventDefault(); setBusy(true); setError(''); setSuccess('');
    try {
      const updated = await api(`/events/${item.id}/assignment`, { method: 'PATCH', token: session.token, body: { assigned_department: department.trim() || null } });
      setItem(updated); setDepartmentDirty(false); setSuccess('Department assignment saved.'); onChange();
  ...
  return <Modal title="Incident details" wide onClose={onClose}>
    ...
    {detailError && <p className="form-error" role="alert">{detailError} <button className="text-button" onClick={() => setReload(value => value + 1)}>Retry details</button></p>}
    ...
    {!demo && canReviewEvents(session) && <section className="detail-section"><h3>Assign a department</h3><form className="status-form" onSubmit={assignDepartment}><input aria-label="Assigned department" maxLength={100} placeholder="e.g. Municipality" value={department} onChange={e => { setDepartment(e.target.value); setDepartmentDirty(true); }} disabled={busy || loading} /><button className="button secondary" disabled={busy || loading || !departmentDirty}>Save assignment</button></form><p className="form-hint">Clear the field to remove the assignment.</p></section>}
    <section className="detail-section"><div className="section-inline"><h3><FileImage size={17} />Evidence</h3>{canViewEvidence(session) && !demo && <button className="button secondary small" disabled={evidenceBusy} onClick={loadEvidence}>{evidenceBusy ? 'Loading…' : evidence ? 'Refresh evidence' : 'Load evidence'}</button>}</div>
      {!canViewEvidence(session) || demo ? <p className="form-hint"><LockKeyhole size={14} />Evidence is available to authorized staff on live incidents.</p> : <>{evidence === null && <p className="form-hint">Load the evidence records linked to this incident.</p>}{evidence?.length === 0 && <p className="form-hint">No evidence has been attached to this incident.</p>}{evidence?.map((entry, index) => <div className="evidence-item" key={entry.id}><FileImage size={18} /><span><strong>Evidence {index + 1}</strong><small>{fullDate(entry.captured_at)}</small></span>{safeEvidenceUrl(entry.file_url) ? <a href={safeEvidenceUrl(entry.file_url)} className="text-button" target="_blank" rel="noopener noreferrer">Open file<ArrowUpRight size={15} /></a> : <span className="muted">Link unavailable</span>}</div>)}</>}
